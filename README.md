...
|Flat Geometry / Texture Cards|🔴 Egg files containing flat geometry or textures pre-generated with Panda3D's `egg-texture-cards` tool (otherwise known as "texture cards") do not have a special 2D importer. They currently can only be imported as a 3D mesh.
...
|Collisions|🔴 No
|Flat Geometry / Texture Cards|🟢 BAM files containing flat geometry or textures pre-generated with Panda3D's `egg-texture-cards` tool (otherwise known as "texture cards") can be imported as 2D nodes. Texture cards, or any square with a 1:1 UV map, will be imported as a **Sprite2D**. Anything more complex can optionally be imported as a **MeshInstance2D**. All flat geometry will be placed under a parent **Node2D** with their position/scaling preserved.<br><br>Detecting BAM file contents is not automatic: you must choose the **BAM Model (2D)** importer or call `BamParser.make_2d()`.<br><br>You can also pull texture resources as they're laid out in the BAM file directly by using `BamParser.make_2d_entries()`, which is more useful for making **Control**-inherited (UI) Godot nodes. However, due to an existing broken implementation for Godot's MeshTexture, complex geometry will still output a **MeshInstance2D** node rather than a texture. For that reason, you must opt-in by passing `include_complex_meshes=true` to the aforementioned method.
