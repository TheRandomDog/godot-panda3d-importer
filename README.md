...
|Texture Cards|🔴 Egg files containing flat geometry with textures pre-generated with Panda3D's `egg-texture-cards` tool (otherwise known as "texture cards") are not currently supported.
...
|Collisions|🔴 No
|Texture Cards|🟢 BAM files containing flat geometry with textures pre-generated with Panda3D's `egg-texture-cards` tool (otherwise known as "texture cards") can be imported as **Sprite2Ds.** Sprites created this way will be parented under a **Node2D**, and sprite's will be automatically positioned/scaled in a way that matches the BAM file.<br><br>Detecting BAM file contents is not automatic: you must choose the **BAM Model as Sprite2Ds** importer or call `BamParser.make_sprites()`.<br><br>You can also pull textures directly from a BAM file matching the same criteria, which is more useful for making **Control**-inherited (UI) Godot nodes, using `BamParser.make_atlas_textures()`.
