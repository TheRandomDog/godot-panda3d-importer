...
var bam_model_importer = preload("./bam/importer_model.gd").new()
#var bam_model_importer_post = preload('./bam/importer_model_post.gd').new()
var bam_font_importer = preload("./bam/importer_font.gd").new()
#var bam_flat_importer = preload("./bam/importer_flat.gd").new()
...
func _enter_tree():
	...
	add_import_plugin(bam_font_importer)
	#add_import_plugin(bam_flat_importer)
	add_import_plugin(egg_anim_importer)
	add_import_plugin(egg_model_importer)
	add_import_plugin(egg_font_importer)
	add_scene_format_importer_plugin(bam_model_importer)
	#add_scene_post_import_plugin(bam_model_importer_post)
...
func _exit_tree():
	...
	remove_import_plugin(bam_font_importer)
	#remove_import_plugin(model_importer)
	#remove_import_plugin(anim_importer)
	remove_import_plugin(sgi_importer)
	remove_scene_format_importer_plugin(bam_model_importer)
	#remove_scene_post_import_plugin(bam_model_importer_post)
