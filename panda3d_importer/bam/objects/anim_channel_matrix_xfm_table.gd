...
func parse_object_data() -> void:
	...
	var new_hpr = datagram.decode_bool()
	bam_parser.ensure(
		new_hpr,
		"Old HPR for animation channels cannot be read yet"
	)
	...
	for i in range(NUM_MATRIX_COMPONENTS):
		var count := datagram.decode_u16()  # Number of frames for this component
		...
		for j in range(count):
			table[j] = datagram.decode_stdfloat()
		tables.append(table)
